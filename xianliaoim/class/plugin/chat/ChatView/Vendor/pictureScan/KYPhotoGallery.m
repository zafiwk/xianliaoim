...
@interface KYPhotoGallery ()<UIScrollViewDelegate,TAPageControlDelegate>
...
//    fromVCSnapShot = [self screenshot];
    ...
    self.pageControl.currentPage = self.initialPageIndex-1;
    self.pageControl.hidesForSinglePage = YES;
    [self.view addSubview:self.pageControl];
    self.pageControl.delegate = self;
    
    
    _photosGalleryScroll.pageControl = self.pageControl;
...
-(void)loadingImage:(UIImageView *)needLoadingImageView withURL:(NSString *)url shouldCompleted:(BOOL)flag{
   ...
   /* BOOL isInCache = [[SDImageCache sharedImageCache]diskImageExistsWithKey:url completion:^(BOOL isInCache) {
    ...
    */
    [[SDImageCache sharedImageCache] diskImageExistsWithKey:url completion:^(BOOL isInCache) {
        if (!isInCache) {
    ...
    if ([url hasPrefix:@"http://"])
    {
         ...
         */
        [needLoadingImageView sd_setImageWithURL:[NSURL  URLWithString:url] placeholderImage:nil options:SDWebImageProgressiveDownload progress:^(NSInteger receivedSize, NSInteger expectedSize, NSURL * _Nullable targetURL) {
             progressView.progress = (CGFloat)receivedSize / expectedSize;
