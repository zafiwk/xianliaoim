...
-(void)viewDidAppear:(BOOL)animated{
    ...
    WKPLog(@"------EM1v1CallViewController---------");
    WKPLog(@"aBtn:%@  sBtn:%@",NSStringFromCGRect(self.answerButton.frame),NSStringFromCGRect(self.hangupButton.frame));
}
...
- (void)hangupAction{
    ...
    [[EMClient sharedClient].callManager endCall:callId reason:reason];
...
- (void)answerAction{
    ...
    if (error) {
        WKPLog(@"接听电话发送了错误:%@",error.errorDescription);
        dispatch_async(dispatch_get_main_queue(), ^{
            if (error.code == EMErrorNetworkUnavailable) {
                [MBProgressHUD showError:NSLocalizedString(@"network.disconnection", @"Network disconnection") toView:self.view];
            }else{
